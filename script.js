...
function checkFirebaseReady() {
    // Ensure Firebase SDK is loaded and an app has been initialized before accessing Realtime Database
    if (typeof firebase !== 'undefined' && firebase.database && Array.isArray(firebase.apps) && firebase.apps.length > 0) {
        try {
            realtimeDb = firebase.database();
            // optional: storage reference
            try { firebaseStorage = firebase.storage(); } catch (e) { console.warn('Firebase storage not available yet', e); }
            firebaseReady = true;
            console.log('✓ Firebase Realtime Database ready');
            return true;
        } catch (e) {
            console.warn('Firebase exists but could not access Realtime Database yet:', e);
            return false;
        }
    }
    return false;
...
// Wait until firebase initialized (simple poll), then call a callback
async function waitForFirebaseInit(timeoutMs = 10000) {
        ...
        const check = () => {
            if (typeof firebase !== 'undefined' && Array.isArray(firebase.apps) && firebase.apps.length > 0) {
                try {
                    ...
                    firebaseReady = true;
                    console.log('✓ Firebase initialized via waitForFirebaseInit');
                    resolve(true);
                    return;
                } catch (e) {
                    console.warn('waitForFirebaseInit: firebase present but access failed', e);
                }
            }
            if (Date.now() - start > timeoutMs) {
                console.warn('waitForFirebaseInit: timeout');
                resolve(false);
                return;
            }
...
// Ensure user is authenticated (anonymous) so Realtime Database/Storage rules that require auth succeed
async function ensureAuth() {
    if (typeof firebase === 'undefined' || !firebase.auth) {
        console.warn('Firebase Auth SDK not available');
        return false;
    }

    const current = firebase.auth().currentUser;
    if (current) {
        console.log('✓ Firebase already authenticated as', current.uid);
        return true;
    }
...
// Sync: fetch Realtime Database data (dossiers, documents, images) and merge into localStorage
async function syncFromFirebaseToLocal() {
    if (!firebaseReady || !realtimeDb) {
        console.warn('syncFromFirebaseToLocal: Firebase not ready');
        return { ok: false, reason: 'firebase-not-ready' };
    }

    try {
        // Dossiers
        const dossierSnap = await realtimeDb.ref('dossiers').once('value');
        const localDossiers = JSON.parse(localStorage.getItem(DOSSIER_KEY)) || [];
        const dossierMap = new Map(localDossiers.map(d => [d.id, d]));
        dossierSnap.forEach(childSnap => {
            const data = { id: childSnap.key, ...childSnap.val() };
            if (!dossierMap.has(childSnap.key)) dossierMap.set(childSnap.key, data);
        });
        const mergedDossiers = Array.from(dossierMap.values());
        localStorage.setItem(DOSSIER_KEY, JSON.stringify(mergedDossiers));

        // Documents
        const docSnap = await realtimeDb.ref('documents').once('value');
        const localDocs = JSON.parse(localStorage.getItem(DOCUMENTS_KEY)) || [];
        const docMap = new Map(localDocs.map(d => [d.id, d]));
        docSnap.forEach(childSnap => {
            const data = { id: childSnap.key, ...childSnap.val() };
            if (!docMap.has(childSnap.key)) docMap.set(childSnap.key, data);
        });
        const mergedDocs = Array.from(docMap.values());
        localStorage.setItem(DOCUMENTS_KEY, JSON.stringify(mergedDocs));

        // Images
        const imgSnap = await realtimeDb.ref('images').once('value');
        const localImgs = JSON.parse(localStorage.getItem(IMAGES_KEY)) || [];
        const imgMap = new Map(localImgs.map(i => [i.id, i]));
        imgSnap.forEach(childSnap => {
            const data = { id: childSnap.key, ...childSnap.val() };
            if (!imgMap.has(childSnap.key)) imgMap.set(childSnap.key, data);
        });
        const mergedImgs = Array.from(imgMap.values());
        localStorage.setItem(IMAGES_KEY, JSON.stringify(mergedImgs));

        console.log('✓ Sync from Firebase -> local complete');
        return { ok: true };
    } catch (e) {
        console.error('syncFromFirebaseToLocal error:', e);
        return { ok: false, reason: e.message };
    }
}

// Sync: push local documents/images (with inline content) to Firebase Storage + Realtime Database
async function syncLocalToFirebase(progressCb) {
    if (!firebaseReady || !realtimeDb || !firebaseStorage) {
        console.warn('syncLocalToFirebase: Firebase not ready');
        return { ok: false, reason: 'firebase-not-ready' };
    }
    ...
    let total = localDocs.length + localImgs.length;
    let done = 0;

    const report = () => {
        if (typeof progressCb === 'function') progressCb(done, total);
    };

    // Upload documents that have content but no downloadURL
    for (const doc of localDocs) {
        if (doc.content && !doc.downloadURL && doc.fileName && doc.content.startsWith('data:')) {
            try {
                const blob = dataURLtoBlob(doc.content);
                const storageRef = firebaseStorage.ref().child(`dossiers/${doc.dossierId}/documents/${doc.id}/${doc.fileName}`);
                await storageRef.put(blob);
                const downloadURL = await storageRef.getDownloadURL();
                doc.downloadURL = downloadURL;
                try { delete doc.content; } catch(e){}
                await realtimeDb.ref('documents/' + doc.id).set(doc);
                console.log('Synced document to Firebase:', doc.id);
            } catch (e) {
    ...
    for (const img of localImgs) {
        if (img.data && !img.downloadURL) {
            try {
                const blob = dataURLtoBlob(img.data);
                const storageRef = firebaseStorage.ref().child(`dossiers/${img.dossierId}/images/${img.id}/${img.code || 'image'}`);
                await storageRef.put(blob);
                const downloadURL = await storageRef.getDownloadURL();
                img.downloadURL = downloadURL;
                try { delete img.data; } catch(e){}
                await realtimeDb.ref('images/' + img.id).set(img);
                console.log('Synced image to Firebase:', img.id);
            } catch (e) {
                console.error('Failed to sync image', img.id, e);
            }
        }
        done++;
        report();
    ...
    // Save updated local copies (metadata updated)
    try { localStorage.setItem(DOCUMENTS_KEY, JSON.stringify(localDocs)); } catch(e) { console.warn('saving docs after sync failed', e); }
    try { localStorage.setItem(IMAGES_KEY, JSON.stringify(localImgs)); } catch(e) { console.warn('saving imgs after sync failed', e); }

    return { ok: true };
}

// Start realtime listeners to keep localStorage and UI in sync with Realtime Database
function startRealtimeListeners() {
    if (!firebaseReady || !realtimeDb) return;

    try {
        // Dossiers
        realtimeDb.ref('dossiers').on('value', snapshot => {
            const dossiers = [];
            snapshot.forEach(childSnap => {
                dossiers.push({ id: childSnap.key, ...childSnap.val() });
            });
            try { localStorage.setItem(DOSSIER_KEY, JSON.stringify(dossiers)); } catch(e) { console.warn('Could not update local dossiers from realtime:', e); }
            if (document.getElementById('dossierContainer')) {
                loadDossiers();
            }
        }, err => console.error('Realtime dossiers error:', err));

        // Documents
        realtimeDb.ref('documents').on('value', snapshot => {
            const docs = [];
            snapshot.forEach(childSnap => {
                docs.push({ id: childSnap.key, ...childSnap.val() });
            });
            try { localStorage.setItem(DOCUMENTS_KEY, JSON.stringify(docs)); } catch(e) { console.warn('Could not update local documents from realtime:', e); }
            // If we're on a dossier page, reload it to show new documents
            if (document.getElementById('dossierTitle')) {
                try { loadDossierPage(); } catch(e) { console.warn('loadDossierPage error after docs snapshot', e); }
            }
        }, err => console.error('Realtime documents error:', err));

        // Images
        realtimeDb.ref('images').on('value', snapshot => {
            const imgs = [];
            snapshot.forEach(childSnap => {
                imgs.push({ id: childSnap.key, ...childSnap.val() });
            });
            try { localStorage.setItem(IMAGES_KEY, JSON.stringify(imgs)); } catch(e) { console.warn('Could not update local images from realtime:', e); }
            if (document.getElementById('dossierTitle')) {
                try { loadDossierPage(); } catch(e) { console.warn('loadDossierPage error after images snapshot', e); }
            }
        }, err => console.error('Realtime images error:', err));

        console.log('Realtime Realtime Database listeners attached');
    } catch (e) {
...
async function getAllDossiers() {
    ...
    // Try Realtime Database and merge only when it returns data
    if (firebaseReady && realtimeDb) {
        try {
            const snapshot = await realtimeDb.ref('dossiers').once('value');
            if (snapshot.exists()) {
                const dossiers = [];
                snapshot.forEach(childSnap => {
                    dossiers.push({id: childSnap.key, ...childSnap.val()});
                ...
                // Realtime Database returned no data: keep local data
                console.log('Realtime Database returned no dossiers; using local storage');
                return localDossiers;
            }
        } catch(e) {
            console.log('Realtime Database unavailable:', e);
            return localDossiers;
        }
    }
...
async function addDossier(dossier) {
    ...
    // Save to Realtime Database (best-effort)
    if (firebaseReady && realtimeDb) {
        try {
            await realtimeDb.ref('dossiers/' + dossier.id).set(dossier);
            console.log('✓ Dossier saved to Realtime Database', dossier.id);
        } catch(e) {
            console.log('Realtime Database save failed:', e);
        }
    }
...
async function updateDossier(id, updatedData) {
    ...
    if (index !== -1) {
        ...
        if (firebaseReady && realtimeDb) {
            try {
                await realtimeDb.ref('dossiers/' + id).update(updatedData);
            } catch(e) {
                console.log('Realtime Database update failed:', e);
            }
        }
    }
}
...
async function deleteDossier(id) {
    ...
    if (firebaseReady && realtimeDb) {
        try {
            await realtimeDb.ref('dossiers/' + id).remove();
            const docsSnapshot = await realtimeDb.ref('documents').orderByChild('dossierId').equalTo(id).once('value');
            docsSnapshot.forEach(childSnap => childSnap.ref.remove());
            const imgsSnapshot = await realtimeDb.ref('images').orderByChild('dossierId').equalTo(id).once('value');
            imgsSnapshot.forEach(childSnap => childSnap.ref.remove());
        } catch(e) {
            console.log('Realtime Database delete failed:', e);
        }
    }
}
...
async function addDocument(dossierId, document) {
    ...
    if (firebaseReady && realtimeDb && typeof firebase !== 'undefined' && firebase.storage) {
        try {
            if (document.fileName && document.content && document.content.startsWith('data:')) {
                ...
                // Remove heavy inline content to save Realtime Database/localStorage space
                try { delete document.content; } catch(e){}
                console.log('Uploaded document to Storage:', document.fileName, downloadURL);
            }

            // Save metadata to Realtime Database
            await realtimeDb.ref('documents/' + document.id).set(document);
            console.log('✓ Document metadata saved to Realtime Database', document.id);
        } catch (e) {
            console.warn('Realtime Database/Storage save failed, will fallback to localStorage:', e);
        }
    }
    ...
    try {
        ...
        if (e && (e.name === 'QuotaExceededError' || e.name === 'NS_ERROR_DOM_QUOTA_REACHED' || e.code === 22)) {
            // Don't throw here — localStorage may be full; we've already saved to Realtime Database/Storage when available.
...
async function updateDocument(dossierId, docId, updatedData) {
    ...
    if (index !== -1) {
        ...
        if (firebaseReady && realtimeDb) {
            try {
                await realtimeDb.ref('documents/' + docId).update(updatedData);
            } catch(e) {
                console.log('Realtime Database update failed:', e);
            }
        }
...
async function deleteDocument(dossierId, docId) {
    ...
    if (firebaseReady && realtimeDb) {
        try {
            await realtimeDb.ref('documents/' + docId).remove();
        } catch(e) {
            console.log('Realtime Database delete failed:', e);
        }
    }
...
async function addImage(dossierId, image) {
    ...
    if (firebaseReady && realtimeDb && typeof firebase !== 'undefined' && firebase.storage) {
        try {
            ...
            // Save metadata to Realtime Database
            await realtimeDb.ref('images/' + image.id).set(image);
            console.log('✓ Image metadata saved to Realtime Database', image.id);
        } catch (e) {
            console.warn('Realtime Database/Storage save failed, will fallback to localStorage:', e);
        }
    }
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // If admin page, show sync button and wire it
    const syncBtn = document.getElementById('syncBtn');
    if (syncBtn) {
        // show button only when admin
        if (isAdmin()) syncBtn.style.display = 'inline-block';
        syncBtn.addEventListener('click', async function() {
            if (!isAdmin()) { alert('Admin only'); return; }
            // wait for firebase
            const ready = await waitForFirebaseInit(10000);
            if (!ready) { alert('Firebase not ready; cannot sync'); return; }

            // show a simple progress alert
            let progressWindowOpened = false;
            const progressCb = (done, total) => {
                if (!progressWindowOpened) {
            ...
            // First push local -> firebase
            await syncLocalToFirebase(progressCb);
            // Then pull firebase -> local to merge any remote-only items
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Try to attach realtime listeners when Firebase becomes available
    (async function attachRealtime() {
        const ready = await waitForFirebaseInit(8000);
        if (ready) {
            // ensure auth before attaching realtime listeners
            try {
                await ensureAuth();
            } catch(e) { console.warn('ensureAuth failed', e); }
            startRealtimeListeners();
        } else {
            console.warn('Realtime listeners not attached: firebase not ready');
        }
    })();
});
