...
// Sync: fetch Firestore data (dossiers, documents, images) and merge into localStorage
...
// Sync: push local documents/images (with inline content) to Firebase Storage + Firestore
async function syncLocalToFirebase(progressCb) {
    if (!firebaseReady || !firestoreDb || !firestoreStorage) {
    ...
    for (const doc of localDocs) {
        if (doc.content && !doc.downloadURL && doc.fileName && doc.content.startsWith('data:')) {
            try {
                ...
                const storageRef = firestoreStorage.ref().child(`dossiers/${doc.dossierId}/documents/${doc.id}/${doc.fileName}`);
                await storageRef.put(blob);
                const downloadURL = await storageRef.getDownloadURL();
                doc.downloadURL = downloadURL;
                try { delete doc.content; } catch(e){}
                await firestoreDb.collection('documents').doc(doc.id).set(doc);
    ...
    for (const img of localImgs) {
        if (img.data && !img.downloadURL) {
            try {
                ...
                const storageRef = firestoreStorage.ref().child(`dossiers/${img.dossierId}/images/${img.id}/${img.code || 'image'}`);
                await storageRef.put(blob);
                const downloadURL = await storageRef.getDownloadURL();
                img.downloadURL = downloadURL;
                try { delete img.data; } catch(e){}
                await firestoreDb.collection('images').doc(img.id).set(img);
